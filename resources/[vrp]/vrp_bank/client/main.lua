    ...
    while true do
      ...
      if(IsNearBank() or IsNearATM()) and not (GetEntityHealth(GetPlayerPed(-1)) == 105) then
        ...
        if IsControlJustPressed(1, 51)  then -- IF INPUT_PICKUP Is pressed 
		  if (IsInVehicle()) then
            TriggerEvent('quantum-notify:notify', 'Banken', 'Du skal forlade dit køretøj først', 'error', 5000)
          else
            if bankOpen then
              closeGui()
              bankOpen = false
...
AddEventHandler('bank:deposit', function(amount)
  if(IsNearBank() == true or depositAtATM == true and IsNearATM() == true or depositAnywhere == true ) then
    if (IsInVehicle()) then
        TriggerEvent('quantum-notify:notify', 'Banken', 'Du kan ikke gøre det fra et køretøj', 'error', 5000)
    else
     TriggerServerEvent("bank:deposit", tonumber(amount))
    end
  else
    TriggerEvent('quantum-notify:notify', 'Banken', 'Du kan kun indsætte penge i banken', 'error', 5000)
  end
end)
...
AddEventHandler('bank:withdraw', function(amount)
  if(IsNearATM() == true or IsNearBank() == true or withdrawAnywhere == true) then
    ...
    TriggerEvent('quantum-notify:notify', 'Banken', 'Du kan kun gøre det i banken eller ved en hæveautomat', 'error', 5000)
  end
end)
  ...
  if(IsNearPlayer(toPlayer) == true or giveCashAnywhere == true) then
    ...
    if (playing ~= false) then
      TriggerServerEvent("bank:givecash", toPlayer, tonumber(amount))
        ...
        TriggerEvent('quantum-notify:notify', 'Banken', 'Denne person er ikke i byen', 'error', 5000)
    end
  else
    TriggerEvent('quantum-notify:notify', 'Banken', 'Denne person er ikke i nærheden', 'info', 5000)
  end
end)
...
AddEventHandler('bank:transfer', function(fromPlayer, toPlayer, amount)
  ...
  if (playing ~= false) then
    ...
    TriggerEvent('quantum-notify:notify', 'Banken', 'Denne person er ikke i byen', 'info', 5000)
  end
end)
