...
local ModderStopping = {}

RegisterNetEvent("stopmod:dmv", function(stopmod)
    local src = source

    if ModderStopping["id:" .. src] == nil then
        ModderStopping["id:" .. src] = stopmod
    else
        DropPlayer(src, 'Trying to cheat are ya?')
    end
end)
...
AddEventHandler('playerDropped', function()
    ...
    ModderStopping["id:" .. src] = nil
end)

function isModder(src, stopMod)
    if(tonumber(ModderStopping["id:" .. src]) == tonumber(stopMod)) then
        return false
    else
        exports['sentinel']:ban(source, 'Lua Menu', 'Blacklisted Trigger', true, true)
        return true
    end
end

RegisterServerEvent("dmv:success")
AddEventHandler("dmv:success", function(stopmod)
    local source = source
    local user_id = vRP.getUserId(source)

    if isModder(source, stopmod) == false then 
