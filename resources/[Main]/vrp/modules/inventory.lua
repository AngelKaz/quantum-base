...
function vRP.openInventory(source)
  ...
  if user_id ~= nil then
    ...
    if data then
      ...
      local menudata = {name=lang.inventory.title(),css={top="75px",header_color="#0d1624"}}
      -- add inventory info
      ...
      local choose = function(player,choice)
        ...
        local choices = vRP.getItemChoices(kitems[choice])
          -- build item menu
          local submenudata = {name=choice,css={top="75px",header_color="#0d1624"}}
...
local function build_itemlist_menu(name, items, cb)
  local menu = {name=name, css={top="75px",header_color="#0d1624"}}
    ...
    if data.inventory ~= nil then
      if not chests[name] then
        ...
        vRP.getSData("chest:"..name, function(cdata)
          ...
          -- open menu
          local menu = {name=lang.inventory.chest.title(), css={top="75px",header_color="#0d1624"}}
          -- take
