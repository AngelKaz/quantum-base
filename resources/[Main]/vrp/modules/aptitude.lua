...
function vRP.varyExp(user_id, group, aptitude, amount)
    ...
    if def ~= nil and uaptitudes ~= nil then
        ...
        if player ~= nil then
            ...
            --- exp
            if amount < 0 then
                TriggerClientEvent('quantum-notify:notify', player, 'Ny Notifikation', lang.aptitude.lose_exp({group_title,aptitude_title,-1*amount}), 'info', 5000)
            elseif amount > 0 then
                TriggerClientEvent('quantum-notify:notify', player, 'Ny Notifikation', lang.aptitude.earn_exp({group_title,aptitude_title,amount}), 'info', 5000)
            end
            --- level up/down
            local new_level = math.floor(vRP.expToLevel(exp))
            local diff = new_level-level
            if diff < 0 then
                TriggerClientEvent('quantum-notify:notify', player, 'Ny Notifikation', lang.aptitude.level_down({group_title,aptitude_title,new_level}), 'info', 5000)
            elseif diff > 0 then
                TriggerClientEvent('quantum-notify:notify', player, 'Ny Notifikation', lang.aptitude.level_up({group_title,aptitude_title,new_level}), 'info', 5000)
            end
        end
    end
end
