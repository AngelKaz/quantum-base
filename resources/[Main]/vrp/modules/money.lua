...
AddEventHandler("vRP:playerJoin",function(user_id,source,name)
    MySQL.Async.fetchAll('SELECT * FROM vrp_user_moneys WHERE user_id = ?', { user_id }, function(result)
        if #result == 0 then
            MySQL.Async.execute("INSERT IGNORE INTO vrp_user_moneys(user_id,wallet,bank) VALUES(@user_id,@wallet,@bank)", {user_id = user_id, wallet = cfg.open_wallet, bank = cfg.open_bank})
            Wait(500)
        end
